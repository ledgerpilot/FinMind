...
def weekly_summary():
    uid = int(get_jwt_identity())
    # 'week' parameter expects YYYY-MM-DD for the start of the week (Monday)
    # If not provided, the service will default to the current week.
    week_start_str = (request.args.get("week") or "").strip() or None
    user_gemini_key = (request.headers.get("X-Gemini-Api-Key") or "").strip() or None
    persona = (request.headers.get("X-Insight-Persona") or "").strip() or None
    ...
    logger.info("Weekly financial summary served user=%s week_start=%s", uid, week_start_str)
